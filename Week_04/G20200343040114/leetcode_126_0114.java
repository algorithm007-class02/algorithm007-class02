...
class Solution {
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        // 已经访问过得单词集合：只找最短路径，所以之前出现过得单词不用出现在下一层。
        ...
        // 是否到达符合条件的层：如果该层添加的某一单词符合目标单词，则说明截止该层的所有借为最短路径，停止循环。
        boolean flag = false;
        while (!queue.isEmpty() && !flag) {
            int size = queue.size();
            /*
             * 该层添加的所有元素：每层必须在所有结果都添加完新的单词之后，再将这些单词统一添加到已使用单词集合 如果直接添加到 visited
             * 中，会导致该层本次结果添加之后的相同添加行为失败 如：该层遇到目标单词，有两条路径都可以遇到，但是先到达的将该单词添加进 visited
             * 中，会导致第二条路径无法添加
             */
