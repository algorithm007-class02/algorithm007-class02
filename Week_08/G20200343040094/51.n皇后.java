...
class Solution {
    public List<List<String>> solveNQueens(int n) {
        List<List<String>> res = new LinkedList<>();
        dfs(res, new LinkedList<>(), n, 0, 0, 0, 0);
        return res;
    }

    private void dfs(List<List<String>> res, List<String> tempList, int n, int row, int cols, int pie, int na) {
        if (row == n) {
            res.add(new LinkedList<>(tempList));
            return;
        }
        ...
        while (availPos != 0) {
            ...
            String rowStr = Integer.toBinaryString((1 << n) | p).substring(1).replace('1', 'Q').replace('0', '.'); // 取最低的n位，皇后落子
            tempList.add(rowStr);
            availPos &= (availPos - 1);
            dfs(res, tempList, n, row + 1, cols | p, (pie | p) << 1, (na | p) >> 1);
            tempList.remove(rowStr);
        }
    }
}
// @lc code=end
